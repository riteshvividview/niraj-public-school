...
export default function ReportsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const redeemed = records.filter(
      (r) => r.receipt.redemptionStatus !== "ready",
    ).length;
    ...
    records.forEach((r) => {
      ...
      categoryRevenue.books += grouped.books.reduce(
        (s, i) => s + i.unitPrice * i.quantity,
        0,
      );
      categoryRevenue.uniform += grouped.uniform.reduce(
        (s, i) => s + i.unitPrice * i.quantity,
        0,
      );
      categoryRevenue.stationery += grouped.stationery.reduce(
        (s, i) => s + i.unitPrice * i.quantity,
        0,
      );
      categoryRevenue.programs += grouped.programs.reduce(
        (s, i) => s + i.unitPrice * i.quantity,
        0,
      );
    });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-sub">
            Payments received vs. items collected / attendees checked in.
          </p>
        </div>
              ...
              <TableBody>
                {records.map((r) => (
                  <TableRow key={r.receipt.id}>
                    ...
                    <TableCell>
                      {new Date(r.receipt.issuedAt).toLocaleDateString("en-IN")}
                    </TableCell>
                    ...
                    <TableCell className="capitalize">
                      {r.receipt.redemptionStatus.replace("-", " ")}
                    </TableCell>
                  </TableRow>
                ))}
                {records.length === 0 ? (
